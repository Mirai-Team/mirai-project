...
int main()
{
	...
	data.open("data/log.txt",std::ios::binary);
	data << "Mirai Encryption example" << std::endl;
	data.close();
	
	//Constructor with key parameter.
	mp::Encryption fileEncrypt("mirai");
	boost::filesystem::path directoryToEncrypt("data/");
	
	//Creation of a file with img/ directory.
	...
	std::string output = fileEncrypt.loadFile(outputFile, "data/log.txt");
